...
class GeneratedPreviewStore:
    ...
    def load_compact(self):
        compact_keys = ("artist", "filename", "subfolder", "type", "timestamp", "viewUrl")
        return {
            "items": [
                {key: item[key] for key in compact_keys if key in item}
    ...
    def _source_image_path(self, item, output_dir=None):
        ...
        if not filename:
            return None
        if file_type == "local":
            return _safe_child_path(self.image_dir, filename, must_exist=True)
        if file_type != "output" or not output_dir:
            return None
        subfolder = str(item.get("subfolder") or "").strip()
        relative = str(Path(subfolder) / filename) if subfolder else filename
        return _safe_child_path(output_dir, relative)
...
def _safe_child_path(root, relative_path, must_exist=False):
